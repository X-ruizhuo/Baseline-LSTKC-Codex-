...
def main_worker(args, cfg):
    ...
    for set_index in range(0, len(training_set)):       
        ...
        model, model_trans, model_trans2 = train_dataset(cfg, args, all_train_sets, all_test_only_sets, set_index, model, model_trans, model_trans2, out_channel,
                                            writer, logger_res=logger_res, scaler=scaler)
...
def train_dataset(cfg, args, all_train_sets, all_test_only_sets, set_index, model, model_trans, model_trans2, out_channel, writer, logger_res=None, scaler=None):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--amp', dest='amp', action='store_true', help='enable mixed precision training for lower GPU memory usage')
    parser.add_argument('--no-amp', dest='amp', action='store_false', help='disable mixed precision training')
    parser.set_defaults(amp=True)
    parser.add_argument('--grad-accum-steps', type=int, default=2, help='gradient accumulation steps for 16G GPU')
